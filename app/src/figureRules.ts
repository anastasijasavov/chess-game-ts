...
export function isKingInCheck(possibleSquares: string[]) {
  if (possibleSquares.length === 0) return false;
  for (let index = 0; index < possibleSquares.length; index++) {
    if (
      hasChildren(possibleSquares[index]) &&
      isCheckMove(possibleSquares[index])
    )
      return possibleSquares[index];
    else continue;
  }
  return false;
}

export function isCheckMove(piece: string) {
  return getByID(getPieceBySquareID(piece)) instanceof King;
}
